  ...
  fs.readFile('tally-file.json', 'utf8', (err, data) => {
    if (err) {
      ...
      return res.status(500).send(
          `Error reading tally-file.json: ${err.message}`);
    }
    ...
    axios
        ...
        .catch((error) => {
          ...
          return res.status(500).send(
              `Error pinning JSON to Pinata: ${error.message}`);
        });
  })
});
...
app.post('/generate-tally', (req, res) => {
  ...
  exec(
      `node build/ts/index.js mergeMessages --poll-id ${
          pollId} -r http://127.0.0.1:8545/`,
      (error, stdout, stderr) => {
        if (error) {
        ...
        exec(
            `node build/ts/index.js mergeSignups --poll-id ${
                pollId} -r http://127.0.0.1:8545/`,
            (error, stdout, stderr) => {
              if (error) {
              ...
              exec(
                  `node build/ts/index.js genProofs -sk ${
                      coordinatorPrivateKey} -zp zkeys/ProcessMessages_10-2-1-2_test/ProcessMessages_10-2-1-2_test.0.zkey -zt zkeys/TallyVotes_10-1-2_test/TallyVotes_10-1-2_test.0.zkey -tw zkeys/TallyVotes_10-1-2_test_js/TallyVotes_10-1-2_test.wasm -pw ./zkeys/ProcessMessages_10-2-1-2_test/ProcessMessages_10-2-1-2_test_js/ProcessMessages_10-2-1-2_test.wasm --poll-id ${
                      pollId} -w true -t tally-file.json --output proofs/`,
                  (error, stdout, stderr) => {
                    if (error) {
                      console.error(`Error in genProofs: ${stderr}`);
                      return res.status(500).send(
                          `Error in genProofs: ${stderr}`);
                    }
                    ...
                    fs.readFile('tally-file.json', 'utf8', (err, data) => {
                      ...
                      axios
                          ...
                          .then((response) => {
                            const cid =
                                response.data
                                    .IpfsHash;  // Get the CID from the response
                            return res.json(
                                {cid});  // Send the CID as a response
                          })
                          .catch((error) => {
                            console.error(
                                `Error pinning JSON to Pinata: ${error}`);
                            return res.status(500).send(
